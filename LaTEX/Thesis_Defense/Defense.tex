...
\begin{frame}{\huge RI-CC2}\large
	...
	\hat{T} = \sum_{ai} t^a_i a_a^\dagger a_i + \frac{1}{4}\sum_{aibj} t_{ij}^{ab} a_a^\dagger a_i a_b^\dagger a_j$
		\item CC2 double amplitudes are similar to MP2
	\end{itemize} 
	\vspace{10pt}
	\begin{table}
		\centering
		...
		CCSD & $\langle \Psi^{a}_{i} | \bar{H} | \Psi_{0} \rangle$ = 0 & $\langle \Psi^{ab}_{ij} | \bar{H} | \Psi_{0} \rangle = 0 $ & $O(N^6)$ & $O(N^{4})$\\ 
		RI-CC2 & $\langle \Psi^{a}_{i} | \bar{H} | \Psi_{0} \rangle$ = 0 & $\displaystyle{t^{ab}_{ij} =\frac{(a b \hat{|} i j)}{\epsilon_i + \epsilon_j - \epsilon_a - \epsilon_b}}$ & $O(N^5)$ & $O(N^{3})$\\\bottomrule
		 \small $\bar{H} = e^{-\hat{T}}\hat{H} e^{\hat{T}} $
		\end{tabular}
	\end{table}
	\vfill
\end{frame}
...
\begin{frame}{\huge Potential Energy Surfaces of Q\textsubscript{0}}\large
	\begin{columns}[t]
		...
		\begin{column}{0.50\textwidth}
			Q\textsubscript{0} has C\textsubscript{S} symmetry
			\begin{itemize}
...
\begin{frame}{\huge Potential Energy Surfaces of Q\textsubscript{1}}\large
	\begin{columns}[t]
		...
		\hfill
		\begin{column}{0.50\textwidth}
			  Similar to Q\textsubscript{0}, though isoprene tail breaks the symmetry.
			\begin{itemize}
	...
	\begin{columns}[c]
		\begin{column}{0.75\textwidth}
			...
			\begin{itemize}
				...
				\item \textbf{Region A}: DBS density far from other electrons. Strong correlation.
